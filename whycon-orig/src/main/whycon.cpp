...
void autocalibration() {
	...
	if (saveVals) {
		...
		for (int b = 0; b < 4; b++) {
			maxEval = -10000000;
			for (int i = 0; i < numBots; i++) {
				eval = sX[b] * currentSegmentArray[i].x
						+ sY[b] * currentSegmentArray[i].y;
				if (eval > maxEval) {
					maxEval = eval;
					index[b] = i;
				}
		...
		if (calibStep == autoCalibrationSteps) {
			for (int i = 0; i < 4; i++) {
				calib[i].x = calib[i].x
						/ (autoCalibrationSteps - autoCalibrationPreSteps);
				calib[i].y = calib[i].y
						/ (autoCalibrationSteps - autoCalibrationPreSteps);
				calib[i].z = calib[i].z
						/ (autoCalibrationSteps - autoCalibrationPreSteps);
...
bool initializeLogging() {
	...
	if (robotPositionLog == NULL) {
		fprintf(stderr,
				"Cannot write to log file %s. Does the \"output\" directory exist?\n",
				logFileName);
		return false;
	}
	return true;
...
void processKeys() {
	...
	if (keys[SDLK_m] && lastKeys[SDLK_m] == false)
		printf("SAVE %03f %03f %03f %03f %03f %03f %03f\n", objectArray[0].x,
				objectArray[0].y, objectArray[0].z, objectArray[0].error,
				objectArray[0].d, currentSegmentArray[0].m0,
				currentSegmentArray[0].m1);
	if (keys[SDLK_n] && lastKeys[SDLK_n] == false)
		printf("SEGM %03f %03f %03f\n", currentSegmentArray[0].x,
				currentSegmentArray[0].y, currentSegmentArray[0].m0);
	...
	for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
		...
		if (family == AF_INET) {
			...
			if (s == 0) {
				...
				printf("\nPossible ip: %s\n", ip);
				if (strncmp(ip, "10", 2) != 0 && strncmp(ip, "127", 3) != 0
						&& strncmp(ip, "172", 3) != 0
						&& strncmp(ip, "192", 3) != 0) {
					printf("\nThat's the one...\n\n");
					return;
...
//initialize logging system, camera and network connection
	...
	if (argc < 2) {
		fprintf(stderr,
				"usage: %s imageSource num_robots\ne.g. %s /dev/video0 1\n",
				argv[0], argv[0]);
		return 0;
	}
	camera = new CCamera();
...
//determine gui size so that it fits the screen
	while (imageHeight / guiScale > screenHeight
			|| imageHeight / guiScale > screenWidth)
		guiScale = guiScale * 2;
...
//setup timers to assess system performance
	...
	while (stop == false) {
		...
		printf(
				"Pattern detection time: %i us. Found: %i Static: %i. Clients %i.\n",
		...
		for (int i = 0; i < numBots && useGui && drawCoords; i++) {
			if (currentSegmentArray[i].valid)
				gui->drawStats(currentSegmentArray[i].minx - 30,
						currentSegmentArray[i].maxy, objectArray[i],
						trans->transformType == TRANSFORM_2D);
		}
			...
			for (int i = 0; i < numBots; i++) {
				...
				if (robotPositionLog != NULL)
					fprintf(robotPositionLog,
							"Frame %i Time %ld Object %03i %03i %.5f %.5f %.5f \n",
							frameID, frameTime, i, currentSegmentArray[i].ID,
							objectArray[i].x, objectArray[i].y,
							objectArray[i].yaw);
			}
			if (moveVal > 0)
				frameID++;
				...
				for (int i = 0; i < numBots; i++) {
					...
					if (robotPositionLog != NULL)
						fprintf(robotPositionLog,
								"Frame %i Time %ld Object %03i %03i %.5f %.5f %.5f \n",
								frameID, frameTime, i,
								currentSegmentArray[i].ID, objectArray[i].x,
								objectArray[i].y, objectArray[i].yaw);
				}
				moveOne = moveVal;
				if (moveVal > 0)
					frameID++;
