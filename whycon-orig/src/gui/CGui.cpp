...
void CGui::drawImage(CRawImage* image) {
	...
	SDL_Surface *imageSDL = SDL_CreateRGBSurfaceFrom(imageSrc->data,
			imageSrc->width, imageSrc->height, imageSrc->bpp * 8,
			imageSrc->bpp * imageSrc->width, 0x000000ff, 0x0000ff00, 0x00ff0000,
			0x00000000);
...
void CGui::guideCalibration(int calibNum, float dimX, float dimY) {
	if (calibNum >= 0 && calibNum <= 4) {
		char ttt[100];
		const char *texty[] = { " Click the circle at the [0.000,0.000].",
...
void CGui::displayHelp(bool displayHelp) {
	const char *helpText[] = { "[h] - to display/hide help,",
			"[l] - draw/hide coordinates,", "[s] - save current image,",
			...
			"[1] - switch to camera coordinage system,", "[r] - recalibrate,", };
...
void CGui::drawStats(int x, int y, STrackedObject o, bool D2) {
	...
	if (D2 == false) {
		sprintf(info, "%.3f %.3f %.3f", o.pitch * (180.0 / M_PI),
				o.roll * (180.0 / M_PI), o.yaw * (180.0 / M_PI));
		text = TTF_RenderUTF8_Blended(smallFont, info, ok_col);
		rect.y = y / scale + 12;
		SDL_BlitSurface(text, NULL, screen, &rect);
	}
	...
	if (fabs(r) < 1) {
		...
		for (float x = sx1; x < sx2; x++) {
			...
			bufp = (Uint8 *) screen->pixels
					+ (int) ((x - sx1) * r + sy1 + 0.5) * screen->pitch
					+ (int) (x + 0.5) * 3;
		...
		for (float y = sy1; y < sy2; y++) {
			Uint8 *bufp;
			bufp = (Uint8 *) screen->pixels + (int) (y + 0.5) * screen->pitch
					+ (int) ((y - sy1) / r + sx1 + 0.5) * 3;
			*(bufp + screen->format->Rshift / 8) = 255;
			*(bufp + screen->format->Gshift / 8) = 0;
			*(bufp + screen->format->Bshift / 8) = 0;
		}
